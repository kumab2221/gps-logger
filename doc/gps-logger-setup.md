...
このプロジェクトの運用デフォルトは 10Hz / 115200bps。systemd 起動時に `--prefer-10hz` を付けると、GPS モジュールが 9600bps に戻っていても可能な範囲で 10Hz / 115200bps へ再設定する。
...
受信だけなら GPS モジュールの `RX` は接続しない。systemd 起動時に 10Hz / 115200bps へ自動復帰させる場合は、GPS `RX` と Raspberry Pi `TXD` も接続する。
...
| GND | GND |
| TX | GPIO15 / RXD |
| RX | 未接続、または自動復帰時は GPIO14 / TXD |

```text
GPS TX → Raspberry Pi RX
GPS RX → 未接続、または自動復帰時は Raspberry Pi TX
```

GPS RX を Raspberry Pi の TX に接続すると、Raspberry Pi 側から文字列や制御データが GPS へ送られることがある。受信ロガーとして使うだけなら未接続の方が安全。`--prefer-10hz` で自動復帰したい場合だけ接続する。
...
[Service]
Type=simple
ExecStart=/usr/bin/python3 /opt/gps-logger/gps_logger.py --port /dev/ttyAMA0 --baud 115200 --prefer-10hz --out /var/log/gps-logger --rotate-sec 300 --fsync-sec 2
Restart=always
RestartSec=2
...
### 12. 起動時に 10Hz / 115200bps へ復帰する

systemd の標準設定では、`--prefer-10hz` により起動時に以下を試す。

1. `115200`、`9600` などの候補から NMEA を読めるボーレートを探す
2. 見つけたボーレートで GPS モジュールへ UBX 設定を送る
3. UART を `115200bps`、更新レートを `10Hz`、NMEA を GGA/RMC 中心へ変更する
4. `115200bps` で受信確認できればそのまま記録する
5. 設定できなければ、読めるボーレートへフォールバックして記録を続ける

この自動復帰を使うには、Raspberry Pi の TXD から GPS モジュールの RX へ配線しておく。GPS RX が未接続でもロガーは受信可能な速度へフォールバックするが、10Hz への再設定はできない。
...
1Hz / 9600bps に固定して使う場合は、`--prefer-10hz` を外し、systemd サービスの `ExecStart` を以下にする。
...
`latest_status.json` の `startup.configured_10hz` が `true` なら、起動時の 10Hz / 115200bps 設定に成功している。
...
2. 10Hz 自動復帰を使う場合は GPS RX を Pi TXD に接続する。受信だけなら未接続にする
3. gpsd を停止する
