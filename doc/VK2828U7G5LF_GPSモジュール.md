...
工場出荷状態のモジュールは 9600bps のことが多い。10Hz / 115200bps へ設定済みなら `115200` で確認する。
...
3. SBAS/MSAS を有効化する
4. 不要な NMEA センテンスを無効化する
5. Raspberry Pi 側のロガー設定も同じボーレートにする
...
- このプロジェクトの運用デフォルトは 10Hz / 115200bps
- 工場出荷状態の 1Hz / 9600bps モジュールを使う場合は、Raspberry Pi 側の `--baud` も 9600 に合わせる
