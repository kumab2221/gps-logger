...
- 起動時に可能な範囲で GPS モジュールを 10Hz / 115200bps へ復帰する
...
tty1 の文字化けを避けるため、CUIライブ表示は英数字中心のASCII表記にしています。日本語の説明ラベルは `latest_status.json` に保存します。
...
受信だけなら GPS の `TX` を Raspberry Pi の `RXD` に接続し、GPS の `RX` は未接続にします。systemd 起動時に 10Hz / 115200bps へ自動復帰させる場合は、GPS `RX` と Raspberry Pi `TXD` も接続します。
